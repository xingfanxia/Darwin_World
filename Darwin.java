...
public class Darwin {
    ...
    try {
      ...
      while (scan.hasNext()) {
        ...
        if (s.equals("WORLD_XY")) {
          ...
          if (scan.hasNextInt()) runSteps = scan.nextInt();
          else {
          ...
          if (scan.hasNext() && scan.next().equals("NUM_CREATURES")) {
            ...
            if (scan.hasNextInt()) numCreatures = scan.nextInt();
            else {
              System.out.println(
                  "error in config file, NUM_CREATURES must be followed by an integer");
              return false;
            }
          } else {
            System.out.println(
                "error in config file, SPECIES_FILE line must be followed by NUM_CREATURES line");
            return false;
          }
          ...
          if (scan.hasNext() && scan.next().equals("PLACEMENT_MODE")) {
            ...
            if (scan.hasNext()) placeMode = scan.next();
            else {
              System.out.println(
                  "error in config file, PLACEMENT_MODE must be followed by RANDOM or SET");
              return false;
            }
          } else {
            System.out.println(
                "error in config file, NUM_CREATURES line must be followed by PLACEMENT_MODE line");
            return false;
          }
          // if here, have set the placement mode for this species (though don't know if it's valid
          // yet)
          ...
          if (placeMode.equals("SET")) {
            ...
            if (scan.hasNextInt()) numToSetPlace = scan.nextInt();
            else {
              System.out.println(
                  "error in config file, PLACEMENT_MODE SET must be followed by an integer");
              return false;
            }
              ...
              if (scan.hasNext() && scan.next().equals("PLACE_CELL")) {
                ...
                if (scan.hasNextInt()) {
                  ...
                  if (scan.hasNextInt()) {
                    ...
                    System.out.println(
                        "error in config file, PLACE_CELL must be followed by 2 integers");
                    return false;
                  }
                } else {
                  System.out.println(
                ...
                System.out.println(
                    "error in config file, need N PLACE_CELL lines after PLACEMENT_MODE SET N");
                return false;
              }
          ...
          // if here, have successfully set a valid placement mode for this species, and added any
          // setPlacement Creatures

          // Now add the rest of the creatures with random placement
          for (int i = numToSetPlace; i < numCreatures; i++) addCreature(curSpec);
    ...
    while (i < args.length) {
      if (args[i].equals("-s")) {
        if (i + 1 > args.length) {
          ...
          return false;
        } else speed = Integer.parseInt(args[i + 1]);
        if (!setPauseTime()) return false;
        ...
        if (i + 1 > args.length) {
          ...
          return false;
        } else randSeed = Integer.parseInt(args[i + 1]);
      } else if (args[i].equals("-g")) {
        if (i + 1 > args.length) {
          System.out.println("option -g requires parameter (true or false)");
          return false;
        } else {
          if (args[i + 1].equals("true")) graphics = true;
          else if (args[i + 1].equals("false")) graphics = false;
          else System.out.println("invalid parameter for option -g (requires true or false)");
        }
  ...
  private static ListIterator<BaseCreature> oneCreatureStep(ListIterator<BaseCreature> it) {
    ...
    if (graphics) world.worldMap().updateSquare(c.getPoint(), " ", c.getDir(), -1);
    c.takeOneTurn();
    if (graphics)
      world
          .worldMap()
          .updateSquare(
              c.getPoint(),
              c.getSpecies().getName().substring(0, 1),
              c.getDir(),
  ...
  private static boolean gameOver() {
    ...
    for (BaseCreature c : pop) {
      if (!c.getSpecies().getName().equals(sp)) return false;
    }
    return true;
  }
  ...
  private static void endOfGeneration() {
    ...
    if (gameOver()) {
      System.out.println(
          "Game over at generation "
              + curGen
              + ", species "
              + pop.getFirst().getSpecies().getName()
              + " wins!");
      if (!graphics) System.exit(1);
      ...
      while (key != 'q') {
        ...
        if (Draw.hasNextKeyTyped()) key = Draw.nextKeyTyped();
      }
      System.exit(1);
    }
    curGen++;
  ...
  public static void main(String[] args) {
    ...
    if (args.length > 0 && args[0].equals("-h")) {
      ...
      System.out.println(
          "-g graphics  :  turns graphics on/off, graphics is either true or false. default=true");
      System.out.println(
          "-s speed     :  sets speed of simulation to int <speed>, valid range is [1-5]. default=3");
      System.out.println(
          "-c configFile: sets file to read configuration from to String <configFile>. default='config.txt'");
      System.out.println(
          "-r randSeed  :  sets random number seed for simluation to int <randSeed>. default uses system time ");
      return;
    }
      ...
      if (Draw.mouseClicked()) {
        while (!Draw.mouseClicked()) {
          ...
          if (Draw.hasNextKeyTyped()) {
            ...
            if (key == 'p') {
              for (int i = 0; i < 10; i++) it = oneCreatureStep(it);
            } else if (key == 'c') it = oneCreatureStep(it);
            else System.out.println("invalid key pressed, ignoring...");
          }
        }
      }
    }
