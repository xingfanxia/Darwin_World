...
public class WorldGraph extends BaseWorldGraph {
    ...
    public void addEdge(Vertex Va, Vertex Vb) {	
        if (!Va.edges.contains(Vb))
            Va.edges.add(Vb);
        if (!Vb.edges.contains(Va))
            Vb.edges.add(Va);
    }
    ...
    public void addCreature(Point p, BaseCreature critter) {
        if (inRange(p))
            vertices[p.x][p.y].critter = critter;
    }

    public BaseCreature getCreature(Point p) {
        if (inRange(p))
            return vertices[p.x][p.y].critter;
        else
            return null;     
    }
	...
	if (d==Direction.NORTH) {
	    ...
	    System.out.println("invalid direction passed to adjPoint");
